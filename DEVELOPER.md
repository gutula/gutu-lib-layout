...
Canonical layout primitives for workspace shells, dashboards, and builder splits.
...
- You need **layout primitives**, **responsive structure**, **shared composition** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/layout` instead of reaching into app-specific internals.
...
| `framework/libraries/layout` | Nested publishable library package. |
| `framework/libraries/layout/src` | Public runtime source and exported modules. |
| `framework/libraries/layout/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `WorkspaceShellLayout`, `DashboardGrid`, `SplitPanelLayout` |
...
- Hosts should import from `@platform/layout`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/layout"]
...
```ts
import { packageId, packageDisplayName, WorkspaceShellLayout } from "@platform/layout";

console.log(packageId, packageDisplayName);
console.log(typeof WorkspaceShellLayout);
```
...
- Publishes 0 public modules from `@platform/layout`.
- Exports 6 named symbols through the public entrypoint, including `packageId`, `packageDisplayName`, `packageDescription`, `WorkspaceShellLayout`, `DashboardGrid`, `SplitPanelLayout`.
